...
import (
	...
	"github.com/hdfchain/hdftime/hdftimed/backend"
	"github.com/hdfchain/hdftime/hdftimed/hdftimewallet"
...
type FileSystem struct {
	...
	wallet *hdftimewallet.DcrtimeWallet // Wallet context.
...
// for all get* calls.
func (fs *FileSystem) lazyFlush(dbts int64, fr *backend.FlushRecord) (*hdftimewallet.TxLookupResult, error) {
...
func New(root, cert, host string, enableCollections bool, passphrase []byte) (*FileSystem, error) {
	...
	// Runtime bits
	hdftimewallet.UseLogger(log)
	fs.wallet, err = hdftimewallet.New(cert, host, passphrase)
	if err != nil {
		return nil, err
	}
